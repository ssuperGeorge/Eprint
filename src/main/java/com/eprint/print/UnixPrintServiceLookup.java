...
import com.eprint.print.CUPSPrinter;
...
public class UnixPrintServiceLookup extends PrintServiceLookup
    ...
    private List<BackgroundLookupListener> lookupListeners = null;
    private static String debugPrefix = "UnixPrintServiceLookup>> ";
    private static boolean pollServices = true;
    private static final int DEFAULT_MINREFRESH = 30;  // 2 minutes
    ...
    public UnixPrintServiceLookup() {
        ...
        if (pollServices) {
            ...
            thr.setName("PrintServiceListener");
            thr.setDaemon(false);
            thr.start();
    ...
    private PrintService[]
        ...
        if (name != null && (defService = getDefaultPrintService()) != null) {
            ...
            List<PrintService> matchedServices = new ArrayList<>();
            ...
            for (int i = 0; i< services.length; i++) {
                services[i] = (PrintService)matchedServices.get(i);
            }
            return services;
        }
    }
    ...
    public PrintService[] getPrintServices(DocFlavor flavor,
        ...
        if (CUPSPrinter.isCupsRunning()) {
            List<PrintService> matchingServices = new ArrayList<>();
            for (int i=0; i<services.length; i++) {
                try {
                    if (services[i].
    ...
    public synchronized void
        ...
        if (printServices != null) {
            ...
            if (lookupListeners == null) {
                lookupListeners = new ArrayList<BackgroundLookupListener>();
    ...
    public void run() {
        ...
        synchronized (this) {
            ...
            for (int i=0; i<lookupListeners.size(); i++) {
                listener =
                    (BackgroundLookupListener)lookupListeners.get(i);
            ...
            while (true) {
                try {
                    ...
                    if(printServices.length==0) {
                    	s_log.warn("No printer is found..  Refreshing print services. ");
                    }
                } catch (Exception se) {
                		s_log.error(debugPrefix+"Exception in refresh thread.");
                }
                if ((printServices != null) &&
